...
public class NetworkVideoPlayer : BaseVideoPlayer {
	...
	*/
	...
	{
		string pipeline="udpsrc port=" + Port.ToString () + " ! application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H265 ! rtpjitterbuffer ! queue !  rtph265depay ! h265parse !  nvh265sldec " +
			"! videoconvert ! video/x-raw, format=I420, framerate=30/1 ! appsink name=videoSink sync=false ";

		/*string pipeline = "udpsrc port=" + Port.ToString() + " ! application/x-rtp ! rtpjitterbuffer ! queue !  rtph264depay ! h264parse !  avdec_h264 " +
			"! videoconvert ! video/x-raw, format=I420, framerate=30/1 ! autovideosink sync=false ";*/
		if (Audio)
			pipeline += "udpsrc port=7000 ! application/x-rtp,media=audio,clock-rate=48000,encoding-name=OPUS,payload=96,encoding-params=2  ! rtpjitterbuffer ! rtpopusdepay ! opusdec ! audioconvert ! audioresample ! autoaudiosink sync=false";
