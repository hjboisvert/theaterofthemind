...
public class NetworkVideoPlayer : BaseVideoPlayer {
	...
	*/
	...
	protected override string _GetPipeline ()
	{
		...
		if (Audio)
			pipeline += "udpsrc port=5001 ! application/x-rtp,media=audio,clock-rate=48000,encoding-name=OPUS,payload=96,encoding-params=2  ! rtpjitterbuffer ! rtpopusdepay ! opusdec ! audioconvert ! audioresample ! autoaudiosink sync=false";
